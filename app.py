...
def load_config():
    ...
    try:
        with open('config.js', 'r') as f:
            ...
            f.read()  # Read file to check if it exists
            # Extract the configuration object (this is a simplified approach)
            # For now, return default config - could be enhanced to parse
            # actual JS
            return {
...
def main():
    # Header
    st.markdown(
        '<h1 class="main-header">🏠 South Florida Property Scraper</h1>',
        unsafe_allow_html=True)
    st.markdown("---")
...
def show_dashboard():
    ...
    load_summary()  # Load summary data to check availability
    ...
    with col3:
        pm_count = len([r for r in results if r.get(
...
def show_scraper_interface():
    st.markdown(
        '<div class="section-header">🚀 Run Scraper</div>',
        unsafe_allow_html=True)

    # Current status
    if is_scraper_running():
        st.markdown(
            '<p class="status-running">🔄 Scraper is currently running...</p>',
            unsafe_allow_html=True)
        ...
        st.markdown(
            '<p class="status-complete">✅ Scraper completed successfully!</p>',
            unsafe_allow_html=True)
        ...
        with col1:
            ...
            st.write(
                f"**Browser Instances:** {config.get('browserInstances', 2)}")
            st.write(
                f"**Request Delay:** {config.get('requestDelay', 2000)}ms")
...
def show_configuration():
    ...
    st.warning(
        "⚠️ Configuration editing via web interface coming soon! For now, edit config.js directly.")
...
def show_data_analysis():
    st.markdown(
        '<div class="section-header">📈 Data Analysis</div>',
        unsafe_allow_html=True)
    ...
    with col2:
        with_phone = len(
            df[df['phone'].notna() & (df['phone'] != 'Send to phone')])
...
def show_export_data():
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"scraped_properties_{
    ...
    with col2:
        ...
        st.write(
            f"**File Size (CSV):** ~{len(csv.encode('utf-8')) // 1024} KB")
        st.write(
